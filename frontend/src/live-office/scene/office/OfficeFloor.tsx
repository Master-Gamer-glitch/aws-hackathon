...
import { buildRoomSign } from './RoomSign';
...
import type { Tile, Facing, ErrandKind, ErrandSpot, StationSpot, StationName } from './themeRegistry';
...
interface Runtime {
  ...
  busySince?: number;
  /** The tool station this agent is currently at / walking to (see stations in the theme). */
  station?: { kind: StationName; spot: StationSpot; key: string };
}
...
const ERRAND_THOUGHTS: Record<ErrandKind, readonly string[]> = {
  ...
  meet:      ['office.errand.meet.0', 'office.errand.meet.1', 'office.errand.meet.2'],
  servers:   ['office.errand.servers.0', 'office.errand.servers.1', 'office.errand.servers.2'],
  sync:      ['office.errand.sync.0', 'office.errand.sync.1', 'office.errand.sync.2'],
};
...
export function OfficeFloor() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Room nameplates (COMMAND CENTER, ENGINEERING, …) — scene elements, not map art.
      for (const sign of theme.signs ?? []) charLayer.addChild(buildRoomSign(sign, calTs));

      // Door posts frame every wall opening (theme.doorways) so rooms read as separate cabins.
      for (const d of theme.doorways ?? []) {
        const post = (x: number, y: number, w: number, h: number): void => {
          const g = new Graphics();
          g.eventMode = 'none';
          g.rect(x, y, w, h).fill(0x5a2b35);                    // dark maroon post
          g.rect(x, y, w, 1).fill(0x8a4453);                    // lit top edge
          g.rect(x + w - 1, y, 1, h).fill(0x3d1c24);            // shaded side
          g.zIndex = y + h;
          charLayer.addChild(g);
        };
        if (d.orient === 'v') {
          post(d.x * calTs + 3, d.y * calTs - 6, 10, 7);                    // above the opening
          post(d.x * calTs + 3, (d.y + d.len) * calTs - 1, 10, 7);          // below the opening
        } else {
          const th = (d.thick ?? 1) * calTs;
          post(d.x * calTs - 4, d.y * calTs - 1, 5, th + 2);                // left of the opening
          post((d.x + d.len) * calTs - 1, d.y * calTs - 1, 5, th + 2);      // right of the opening
        }
      }
      ...
      const GOD_SEAT = 0;
      // Role → room: a theme may pin an agent to the desk of its department
      // (theme.agentSeats: agent id → spawn-point name). Those home desks are held back
      // from agents without a mapping (e.g. hired later), who take spare workstations.
      const seatIndexOf = (name: string | undefined): number => {
        const p = name ? mapRenderer.getSpawnPoint(name) : undefined;
        return p ? seatTiles.findIndex((s) => s.x === p.x && s.y === p.y) : -1;
      };
      const reservedSeats = new Set<number>(
        Object.values(theme.agentSeats ?? {}).map(seatIndexOf).filter((i) => i > 0)
      );
      const claimSeat = (agent: Agent): number | null => {
        if (agent.isGod) { seatClaims.add(GOD_SEAT); return GOD_SEAT; }
        const home = seatIndexOf(theme.agentSeats?.[agent.id]);
        if (home > 0 && !seatClaims.has(home)) { seatClaims.add(home); return home; }
        for (let i = 1; i < seatTiles.length; i++) {
          if (!seatClaims.has(i) && !reservedSeats.has(i)) { seatClaims.add(i); return i; }
        }
        for (let i = 1; i < seatTiles.length; i++) {
      ...
      const machineTile: Tile = theme.coffee.machineTile ?? { x: 26, y: 17 };
      machineG.position.set(machineTile.x * ts0, machineTile.y * ts0);
      machineG.zIndex = (machineTile.y + 2) * ts0;
      ...
      const updateErrands = (dt: number): void => {
        for (const [errId, rt] of runtimes) {
          const err = rt.err;
          if (!err) continue;
          err.timer += dt;
          const spot = ERRAND_SPOTS[err.idx];
          // the lead heads back to the command center; everyone else resumes roaming
          const resume = (): void => {
            if (agentById(errId)?.isGod) rt.character.sitAtDesk(true);
            else rt.character.startWandering();
          };
          if (err.phase === 'walking') {
            if (err.timer > 20) { releaseErrand(rt); resume(); }
            continue;
          }
      ...
      const updateBossAura = (dt: number): void => {
        ...
        if (statsAge > 30) {
          statsAge = 0;
          if (window.cth?.hiveTasks) {
            void window.cth.hiveTasks().then((raw: any) => {
      ...
      // The wall clock beside the lead's window is the "closing time" entry. On the
      // desktop host it ran the real close flow; on the web there is no window to
      // close, so it raises a `cth:clock-out` event the CrewDesk shell listens for.
      ...
      clockG.on('pointertap', (ev) => {
        ev.stopPropagation();
        window.dispatchEvent(new CustomEvent('cth:clock-out'));
      });
      ...
      askG.cursor = 'pointer';
      const askAnchor = theme.anchors.askBoard ?? { px: 14 * tsB + 25, y: 10 };
      askG.position.set(askAnchor.px, askAnchor.y * tsB);
      askG.zIndex = (askAnchor.y + 1) * tsB;
      ...
      const boardStands = theme.anchors.boardStands
        ?? { pin: { x: 8, y: 11 }, take: { x: 9, y: 11 }, archive: { x: 12, y: 11 } };
      const PIN_STAND: Tile = boardStands.pin;         // under the blockers board
      const TAKE_STAND: Tile = boardStands.take;       // under the todo board
      const ARCHIVE_STAND: Tile = boardStands.archive; // beside the archive table
      ...
      const pollTaskBoard = async (): Promise<void> => {
        try {
          const raw = (window.cth?.hiveTasks ? await window.cth.hiveTasks() : null) as { tasks?: Array<{ id?: string; status?: string; assignee?: string; humanQA?: Array<{ q?: string; a?: string }> }> } | null;
      ...
      const removeCharacter = (id: string) => {
        ...
        releaseStation(rt);              // and any tool station it was using
      ...
      // ─── Tool stations: work happens where the tool lives ─────────────────
      // While an agent is `thinking`/`working` at a station other than its own desk
      // (a spare workstation, a bookshelf, the meeting-room board, the server racks…)
      // it walks there, stays for the job, and walks home when the job ends. The theme
      // decides where each station is; the nearest free one to the agent's desk wins.
      const stationTaken = new Map<string, string>();        // spot key → agent id
      const releaseStation = (rt: Runtime): void => {
        if (rt.station) { stationTaken.delete(rt.station.key); rt.station = undefined; }
      };
      const chooseStation = (agent: Agent, rt: Runtime): Runtime['station'] => {
        const kind = agent.currentStation as string | undefined;
        if (!kind || kind === 'desk') return undefined;
        const list = theme.stations?.[kind as StationName];
        if (!list?.length) return undefined;
        if (rt.station?.kind === kind) return rt.station;    // keep the spot already picked
        const home = rt.character.getDeskTile();
        const free = list.filter((sp) => {
          const key = `${kind}:${sp.stand.x},${sp.stand.y}`;
          const holder = stationTaken.get(key);
          if (holder && holder !== agent.id) return false;
          if (sp.stand.x === home.x && sp.stand.y === home.y) return false;   // not your own desk
          // never take over another agent's home desk
          for (const [id, other] of runtimes) {
            const d = other.character.getDeskTile();
            if (id !== agent.id && d.x === sp.stand.x && d.y === sp.stand.y) return false;
          }
          return true;
        });
        if (free.length === 0) return undefined;
        const dist = (sp: StationSpot) => Math.abs(sp.stand.x - home.x) + Math.abs(sp.stand.y - home.y);
        free.sort((a, b) => dist(a) - dist(b));
        const spot = (free.length > 1 && Math.random() < 0.3) ? free[1] : free[0];
        const key = `${kind}:${spot.stand.x},${spot.stand.y}`;
        stationTaken.set(key, agent.id);
        return { kind: kind as StationName, spot, key };
      };
      const goToStation = (rt: Runtime, st: NonNullable<Runtime['station']>): void => {
        const c = rt.character;
        c.walkToAndThen(st.spot.stand, () => {
          if (rt.station !== st) return;                       // job changed while walking
          if (st.spot.sit) c.sitInPlace(st.spot.facing);
          else { c.setIdle(); c.faceDirection(st.spot.facing); }
        });
      };
      ...
      const applyState = (agent: Agent, rt: Runtime, force = false) => {
        ...
        // A finished (or re-targeted) job frees the station it was using.
        if (rt.station && !(isBusy && (agent.currentStation as string | undefined) === rt.station.kind)) releaseStation(rt);
        ...
        switch (agent.status) {
          ...
          case 'thinking': {
            ...
            const st = chooseStation(agent, rt);
            if (st) { rt.station = st; goToStation(rt, st); }
      ...
      const offMessage = window.cth?.onHiveMessage
        ? window.cth.onHiveMessage((e: any) => {
