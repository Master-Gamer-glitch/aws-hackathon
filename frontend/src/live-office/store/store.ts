...
import type { AgentProvider } from '@office/shared/agentProvider';
import type { HireManifest } from '@office/shared/hire';
import {
  EMPTY_HIRE_QUEUE,
  clearHireQueue,
  enqueueHires,
...
} from '@office/shared/hireQueue';
import { DEFAULT_ORG_TRIGGER, type OrgTriggerConfig, type WebhookTrigger } from '@office/shared/triggers';
import { isCompactionCommand } from '@office/shared/providerAutomation';
import { preferredAgentRole } from '@office/shared/agentRole';
import { isInboxNudge } from '@office/shared/hiveNudge';
import { refocusAfterRemoval, focusOnLoad, restoreFocus } from './focusMode';
import { chooseRosterSource } from './rosterSource';
import { CREW_AGENTS } from '../crewdesk/roster';
...
const LS_SIDEBAR_WIDTH = 'crewdesk.office.sidebarWidth';
const LS_SIDEBAR_TAB = 'crewdesk.office.sidebarTab';
const LS_AGENTS = 'crewdesk.office.agents';
const LS_ARCHIVED = 'crewdesk.office.archivedAgents';
const LS_RESTORABLE = 'crewdesk.office.restorableAgents';
const LS_SELECTED = 'crewdesk.office.selectedId';
const LS_QUEUES = 'crewdesk.office.messageQueues';
/** Which hive this origin's roster keys were last written for. See rosterSource.ts. */
const LS_ROSTER_HOME = 'crewdesk.office.rosterHome';
const LS_FOCUS_MODE = 'crewdesk.office.prefersFocusMode';
...
// The default roster is CrewDesk's six-agent crew (see crewdesk/roster.ts).
const DEFAULT_CREW_AGENTS: Agent[] = CREW_AGENTS;

function loadPersistedAgents(): Agent[] {
  try {
    const parsed = persistedSlice(LS_AGENTS, fileRoster?.agents);
    if (!parsed.length) return DEFAULT_CREW_AGENTS;
